...
hammmingmat <- hamming.distance(binmat) %>% dist()
mantel.rtest(geogdist_staph,hammmingmat,nrepet = 9999)
```

#functions for looking at geog distance
```{r}
NYC_CC8 <- inner_join(NYCdata_SRA, Staph_betas, by = "Run") %>% filter(CC_8 > .8) %>% select(Logitude,Latitude)
avg_geog_dist(NYC_CC8)
set.seed(45345)
rand_dists <- rand_distances(nrow(NYC_CC8),points_staph,1000)
ecdf(rand_dists)(avg_geog_dist(NYC_CC8))
